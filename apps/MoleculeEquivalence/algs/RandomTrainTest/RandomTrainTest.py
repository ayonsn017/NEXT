"""
script for the random train random test condition
...
class MyAlg:
    """
    class to present questions generated at random from both the training and
    test distribution
    """

    increment_dictionary = {parameters.pretest_seed_key: 1,
                            parameters.training_seed_key: 1,
    ...
    def initExp(self, butler, pretest_count, training_count, posttest_count,
        ...
        :return bool: True to indicate that algorithm initialization was a
        success
        """
        butler.algorithms.set(key=parameters.pretest_count_key,
                              value=pretest_count)
        butler.algorithms.set(key=parameters.training_count_key,
                              value=training_count)
        butler.algorithms.set(key=parameters.posttest_count_key,
                              value=posttest_count)
        butler.algorithms.set(key=parameters.guard_gap_key, value=guard_gap)

        # count total number of questions to show
        guard_count = (pretest_count + training_count +
                       posttest_count) / guard_gap
        total_questions = pretest_count + training_count + \
            posttest_count + guard_count
        butler.algorithms.set(key=parameters.total_questions_key,
        ...
        # save the pretest, training, posttest, guard data, time required to
        # finish the survey and monetary gain
        butler.algorithms.set(key=parameters.pretest_dist_key,
                              value=alg_params[parameters.pretest_dist_key])
        butler.algorithms.set(key=parameters.training_data_key,
                              value=alg_params[parameters.training_data_key])
        butler.algorithms.set(key=parameters.posttest_dist_key,
                              value=alg_params[parameters.posttest_dist_key])
        butler.algorithms.set(key=parameters.guard_data_key,
                              value=alg_params[parameters.guard_data_key])
        butler.algorithms.set(key=parameters.time_required_key,
                              value=alg_params[parameters.time_required_key])
        butler.algorithms.set(key=parameters.monetary_gain_key,
                              value=alg_params[parameters.monetary_gain_key])
    ...
    def getQuery(self, butler, participant_uid):
        ...
        :return
            str, representation1 || '_' || molecule1, the first molecule
            str, representation2 || '_' || molecule2, the second molecule
            int, 1 if the two molecules are the same, 0 otherwise
        ...
        # posttest based on this value
        participant_questions_key = \
            utility.gen_participant_questions_key(participant_uid)
        num_reported_answers_key = \
            utility.gen_num_reported_answers_key(participant_uid)

        # new participant
        if not butler.algorithms.exists(key=participant_questions_key):
            # get the question counts
            pretest_count = \
                butler.algorithms.get(key=parameters.pretest_count_key)
            training_count = \
                butler.algorithms.get(key=parameters.training_count_key)
            posttest_count = \
                butler.algorithms.get(key=parameters.posttest_count_key)
            guard_gap = \
                butler.algorithms.get(key=parameters.guard_gap_key)

            # get the data, time required to finish the survey and monetary
            # gain
            pretest_dist = \
                butler.algorithms.get(key=parameters.pretest_dist_key)
            training_dist = \
                butler.algorithms.get(key=parameters.training_data_key)
            posttest_dist = \
                butler.algorithms.get(key=parameters.posttest_dist_key)
            guard_data = \
                butler.algorithms.get(key=parameters.guard_data_key)
            time_required = \
                butler.algorithms.get(key=parameters.time_required_key)
            monetary_gain = \
            ...
            participant_questions = self.generate_all_questions(
                pretest_dist, training_dist, posttest_dist, guard_data,
                pretest_seed, training_seed, posttest_seed, pretest_count,
                training_count, posttest_count, guard_gap, time_required,
            ...
            butler.algorithms.set(key=num_reported_answers_key,
                                  value=num_reported_answers)
        else:
            # get the participant information
            participant_questions = \
                butler.algorithms.get(key=participant_questions_key)
            num_reported_answers = \
                butler.algorithms.get(key=num_reported_answers_key)
    ...
    def processAnswer(self, butler, participant_uid, target_winner):
        ...
        num_reported_answers_key = \
            utility.gen_num_reported_answers_key(participant_uid)
        num_reported_answers = \
            butler.algorithms.increment(key=num_reported_answers_key)
    ...
    def generate_all_questions(self, pretest_dist, training_dist,
                               posttest_dist, guard_data, pretest_seed,
                               training_seed, posttest_seed, pretest_count,
        ...
        :param training_dist: string, the training distribution in string
        format
        :param posttest_dist: string, the posttest distribution in string
        format
        ...
        :param training_count: int, the number of training questions to
        generate
        :param posttest_count: int, the number of posttest questions to
        generate
        :param guard_gap: int, the gap between guard questions
        :param time_required: str, the time required to complete the survey,
        showed in introduction
        :param monetary_gain: str, the monetary gain to the participants
        :param total_questions: int, total number of molecule questions to
        show to the participant
        :return: ParticipantInfo: all questions generated for the participant
        put in a PariticipantInfo object
        """
        participant_questions = []

        '''
        # not showing first instruction question since we don not need consent
        # for MTurkers
        # introduction instruction 1
        # same structure as molecule question. m1 contains the instruction.
        # m2 is empty. same is set to 0. ques_count also set to zero
        participant_question = \
            [instructions.get_introduction_instruction1(monetary_gain), '', 0,
        ...
        guard_question_generator = \
            FixedInstanceReader.FixedInstanceReader(guard_data)
        ...
        training_question_generator = \
            RandomInstanceGenerator.RandomInstanceGenerator(training_dist,
                                                            seed=training_seed)
        participant_questions, index = utility.gen_participant_questions(
